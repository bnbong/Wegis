...
(function () {
  ...
  function normalizeVerdict(apiData, context = {}) {
    ...
    const status =
      typeof data.status === 'string'
        ...
        : 'final';
    const source =
      typeof data.source === 'string' ? data.source.toLowerCase().trim() : '';

    // Map to the internal verdict. Prefer the new severity/status/source
    // contract; fall back to the explicit `verdict` field, then the legacy
    // boolean `result` (which equals "severity == block").
    let verdict;
    if (source === 'error') {
      verdict = VERDICTS.ERROR;
    } else if (status === 'pending') {
      // Not analyzed yet — unknown, NOT confirmed-safe.
      ...
      VERDICT_VALUES.includes(data.verdict.toLowerCase().trim())
    ) {
      verdict = data.verdict.toLowerCase().trim();
    ...
    // dangerous verdict. Downgrade to the explicit unsupported state.
    if (
    ...
    return {
      verdict,
      // Pass the server's own fields through so callers can gate on them.
  ...
  /** True when `source` is an authoritative threat feed (blacklist/reputation). */
  function isAuthoritativeBlockSource(source) {
    const s = typeof source === 'string' ? source.toLowerCase().trim() : '';
    return s === 'blacklist' || s.startsWith('reputation:');
  }

  /**
   * Should navigation to this verdict be hard-blocked? Honors the
   * `blockPhishing` setting. Only `block` severity (mapped to PHISHING) is ever
   * hard-blocked; `warn` (SUSPICIOUS) and uncertain verdicts are soft indicators
   * only, never a hard interstitial.
   *
   * For discovered links (any `sourceType` other than `navigation`) the client
   * additionally requires an authoritative `source` (blacklist / reputation:*)
   * before hard-blocking — a defense-in-depth match to the server contract that
   * a link's model/cache signal never yields `block`. The active page
   * (`sourceType=navigation`) CAN be hard-flagged by the model, so it is exempt.
   * `opts` is optional; without it (e.g. unit checks) the source gate is skipped.
   */
  function shouldBlock(verdict, settings = {}, opts = {}) {
    if (settings.blockPhishing === false) {
      return false;
    }
    if (verdict !== VERDICTS.PHISHING) {
      return false;
    }
    const sourceType = opts.sourceType;
    const isDiscoveredLink = sourceType && sourceType !== 'navigation';
    if (isDiscoveredLink) {
      return isAuthoritativeBlockSource(opts.source);
    }
    return true;
  }
